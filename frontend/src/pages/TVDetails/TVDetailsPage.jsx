...
export default function TVDetailsPage() {
  ...
  useEffect(() => {
    ...
    const loadTVDetails = async () => {
      ...
      setError("");
      try {
        const response = await discoverApi.tvDetails(id);
        if (!active) return;
        setItem(response?.data?.tv || null);
      } catch (err) {
        if (!active) return;
  ...
  const handlePlayTrailer = async () => {
    ...
    try {
      const response = await discoverApi.tvTrailer(id);
  ...
  return (
    <PageFrame tone="blue">
      <div className="mt-8 rounded-[30px] border border-white/12 bg-black/35 p-5 backdrop-blur-sm md:p-7">
        ...
        {loading && <CinematicLoader label="Loading TV details" />}
        ...
        <div className="mt-5 grid gap-6 md:grid-cols-[260px_1fr]">
          <div className="overflow-hidden rounded-2xl border border-white/15 bg-slate-900">
            {posterUrl ? (
              <img src={posterUrl} alt={item?.title || `TV ${id}`} className="h-full w-full object-cover" />
            ) : (
          ...
          <div>
            <p className="inline-flex rounded-full border border-violet-300/30 bg-violet-400/10 px-3 py-1 text-xs text-violet-100">
              TV Show Details
            </p>
            <h1 className="mt-4 text-3xl font-black md:text-5xl">{item?.title || `TV Show ${id}`}</h1>
        ...
        <div className="mt-8 rounded-2xl border border-white/15 bg-slate-900/50 p-4 md:p-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <button
              ...
              className="rounded-lg border border-violet-300/40 bg-violet-400/20 px-4 py-2 text-sm text-violet-100 hover:bg-violet-400/30 disabled:opacity-60"
            >
            ...
            <div className="mt-4 overflow-hidden rounded-xl border border-white/10 bg-black">
              {trailerEmbedUrl ? (
                <iframe
                  ...
                  title={`${item?.title || "TV Show"} trailer`}
                  allow="autoplay; encrypted-media; picture-in-picture"
                  className="aspect-video w-full"
                />
              ) : (
                <p className="p-4 text-sm text-slate-300">
                  Trailer for this TV show is currently unavailable.
                </p>
              )}
            </div>
          )}
